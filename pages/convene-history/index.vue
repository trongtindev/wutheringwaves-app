...
const loadStatistics = () => {
  ...
  convenes.value = result.map((e) => {
    ...
    newObject.timeConverted = moment(e.time).utcOffset(account.timeOffset);
    return newObject;
  }) as any;
    ...
    .filter((e) => {
      if (filterBanner.value) {
        if (
          ...
          const timeStart = moment(filterBanner.value.time.start)
            .utcOffset(8)
            .add(account.timeOffset - 8, 'hours');
          const timeEnd = moment(filterBanner.value.time.end)
            .utcOffset(8)
            .add(account.timeOffset - 8, 'hours');
